...
这些参数影响的是"怎么选词"，不影响模型理解问题的能力。在聊天界面里你通常看不到它们——它们被隐藏了，对大多数读者来说，不需要手动调整这些参数；知道它们存在即可。但在用 API 构建应用时，可以调整它们来控制输出特性。Agent 框架也会帮你管理这些参数，所以大多数时候你不需要手动设置。
...
一条 Prompt 做所有事，结果往往是：AI 要么顾此失彼，要么理解偏差被放大。这里注意于 Context Bloat 的区别，Context Bloat 是塞了太多背景材料，Mega-Prompt 是在一条 Prompt 里堆了太多指令。
